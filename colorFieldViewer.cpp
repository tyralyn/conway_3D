...
void addBlock(int x, int y) {
  values[zLayer](x,y) = 1.0;
  values[zLayer](x+1,y) = 1.0;
  values[zLayer](x,y+1) = 1.0;
  values[zLayer](x+1,y+1) = 1.0;
  values[zLayer+1](x,y) = 1.0;
  values[zLayer+1](x+1,y) = 1.0;
  values[zLayer+1](x,y+1) = 1.0;
  values[zLayer+1](x+1,y+1) = 1.0;
}

void addBeehive(int x, int y){
  values[zLayer](x,y) = 1.0;
  values[zLayer](x+1,y+1) = 1.0;
  values[zLayer](x+2,y+1) = 1.0;
  values[zLayer](x+3,y) = 1.0;
  values[zLayer](x+1,y-1) = 1.0;
  values[zLayer](x+2,y-1) = 1.0;

  values[zLayer+1](x,y) = 1.0;
  values[zLayer+1](x+1,y+1) = 1.0;
  values[zLayer+1](x+2,y+1) = 1.0;
  values[zLayer+1](x+3,y) = 1.0;
  values[zLayer+1](x+1,y-1) = 1.0;
  values[zLayer+1](x+2,y-1) = 1.0;
}
...
void addToad(int x, int y) {
  ...
  values[zLayer](x,y) = 1.0;
  values[zLayer](x,y+1) = 1.0;
  values[zLayer](x-1,y-2) = 1.0;
  values[zLayer](x-1,y-1) = 1.0;
  values[zLayer](x-1,y-0) = 1.0;
    values[zLayer+1](x,y-1) = 1.0;
  values[zLayer+1](x,y) = 1.0;
  values[zLayer+1](x,y+1) = 1.0;
  values[zLayer+1](x-1,y-2) = 1.0;
  values[zLayer+1](x-1,y-1) = 1.0;
  values[zLayer+1](x-1,y-0) = 1.0;
}

void addGlider(int x, int y) {
  values[zLayer](x,y) = 1.0;
  values[zLayer](x,y+1) = 1.0;
  values[zLayer](x,y+2) = 1.0;
  values[zLayer](x-1,y) = 1.0;
  values[zLayer](x-2,y+1) = 1.0;
    values[zLayer+1](x,y) = 1.0;
  values[zLayer+1](x,y+1) = 1.0;
  values[zLayer+1](x,y+2) = 1.0;
  values[zLayer+1](x-1,y) = 1.0;
  values[zLayer+1](x-2,y+1) = 1.0;
}

void exampleItems() {
  addBlock(xDim/4,yDim/4);
  addBeehive(3*xDim/4, yDim/4);
  addToad(3*xDim/4, 3*yDim/4);
  addGlider(xDim/4, 3*yDim/4);
}
...
void tinyPrimordialCubeCentered() {
  values[zDim/2+1](xDim/2+1,yDim/2) = 1.0;
  values[zDim/2+1](xDim/2,yDim/2) = 1.0;
...
void liveCell(int x, int y, int z) {
  ...
  if (values2[z](x,y) < .5) values2[z](x,y) =1.00;
}

void killCell(int x, int y, int z) {
    if (values2[z](x,y) > 0.5) values2[z](x,y) =0.00;
}
...
{   
  ...
  // Other Transformations
    ...
    for (int i = 0; i < xDim; i ++) {
      for (int j = 0; j < yDim; j++) {
        for (int k = 0; k < zDim; k++) {
          if (isAlive(i,j,k) ) drawCube(i-xDim/2,j-yDim/2,k-zRes/2, .75, (float)(i+10.0)/(float)xDim, (float)(j+10.0)/(float)yDim, (float)(k+10.0)/(float)zDim);
        }
      }
    }
 }
