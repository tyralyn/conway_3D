...
void glutKeyboard(unsigned char key, int x, int y)
{
  switch (key)
  {
    ...
    case 'b':
      ...
      break;      
    case 'c':
      cout<<"C "<<conways3D<<endl;
      if (!conways3D) break;
      for ( int x = 0; x < xRes; x++ ) {
        for ( int y = 0; y < yRes; y++ ) {
          //cout<<values[1](xField,yField)<<" "<<values[2](xField,yField)<<endl;
          if ( values2[1](xField,yField) > 0.5 ) {
            values2[2](xField,yField) = 1.0;
            cout<<x<<" "<<y<<" one\n";
          }
          if ( values2[2](xField,yField) > 0.5 ) {
            values2[1](xField,yField) = 1.0;
            cout<<x<<" "<<y<<" two\n";
          }
        }
      }
...
void drawFrontSquare( float xOffset, float yOffset,float zOffset, float alpha, float R, float G, float B ) {
  ...
  glColor4f( R, G, B, alpha );     glVertex3f(  0.5+xOffset,  0.5+yOffset, -0.5+zOffset );      // P2 is green
  glColor4f( R, G, B, alpha );     glVertex3f( -0.5+xOffset,  0.5+yOffset, -0.5+zOffset );      // P3 is blue
  glColor4f( R, G, B, alpha );     glVertex3f( -0.5+xOffset, -0.5+yOffset, -0.5+zOffset );      // P4 is purple
  glEnd();
}
...
{   
  ...
  // Other Transformations
    ...
    for (int i = 0; i < xDim; i ++) {
      for (int j = 0; j < yDim; j++) {
        for (int k = 0; k < zDim; k++) {
          if (isAlive(i,j,k) ) drawCube(i-xDim/2,j-yDim/2,k-zRes/2, 0.15, (float)(i+10.0)/(float)xDim, (float)(j+10.0)/(float)yDim, (float)(k+10.0)/(float)zDim);
        }
      }
    }
 }
